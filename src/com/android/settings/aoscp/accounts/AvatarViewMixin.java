...
public class AvatarViewMixin implements LifecycleObserver, OnClickListener, Observer {
    ...
    private final ActivityManager mActivityManager = ((ActivityManager) mContext.getSystemService(ActivityManager.class));
	...
	public void onClick(View view) {
		try {
            ...
            FeatureFactory.getFactory(mContext).getMetricsFeatureProvider().action(0, 1643, 1502, null, Integer.MIN_VALUE);
        ...
        if (!TextUtils.isEmpty(provider)) {
            ThreadUtils.postOnBackgroundThread(new Runnable() {
				...
				public final void run() {
					Bundle caller = mContext.getContentResolver().caller(new Builder().scheme("content").authority(str).build(), "getAccountAvatar", null, null);
