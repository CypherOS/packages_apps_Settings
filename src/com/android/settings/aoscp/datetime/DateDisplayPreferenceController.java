...
public class DateDisplayPreferenceController extends AbstractPreferenceController implements
    ...
    public void updateState(Preference pref) {
        final ListPreference mDateDisplay = (ListPreference) pref;
        final Resources res = mContext.getResources();
        if (mDateDisplay != null) {
			mDateDisplay.setOnPreferenceChangeListener(this);
			mDateDisplay.setValue(Integer.toString(Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.STATUSBAR_CLOCK_DATE_DISPLAY, 0)));
			mDateDisplay.setSummary(mDateDisplay.getEntry());
        }
    }

    @Override
    public boolean onPreferenceChange(Preference pref, Object newValue) {
        if (pref == mDateDisplay) {
            int val = Integer.parseInt((String) newValue);
            int index = mDateDisplay.findIndexOfValue((String) newValue);
            Settings.System.putInt(mContext.getContentResolver(),
                    Settings.System.STATUSBAR_CLOCK_DATE_DISPLAY, val);
            mDateDisplay.setSummary(mDateDisplay.getEntries()[index]);
            return true;
		}
        return false;
    }
