...
public class NotificationsAdvanced extends SettingsPreferenceFragment implements OnPreferenceChangeListener {
	...
	private void updateTickerModeSummary(int value) {
        ...
        if (value == 0) {
            // Ticker Disabled
            mTickerNotifications.setSummary(res.getString(R.string.ticker_disabled_summary));
        } else if (value == 1) {
            // Ticker Enabled
            mTickerNotifications.setSummary(res.getString(R.string.ticker_enabled_summary));
		} else if (value == 2) {
            // Ticker Enabled with music capability
            mTickerNotifications.setSummary(res.getString(R.string.ticker_enabled_music_summary));
        } else {
            String mode = res.getString(value == 1
                    ? R.string.ticker_enabled_summary
                    : R.string.ticker_enabled_music_summary);
