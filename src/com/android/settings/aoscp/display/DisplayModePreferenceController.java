...
public class DisplayModePreferenceController extends BasePreferenceController {
    ...
    public CharSequence getSummary() {
        final int currentMode = mHwManager.getCurrentDisplayMode();
        if (currentMode == 3) {
            return mHwManager.getDisplayModeName(3);
        }
        if (currentMode == 2) {
            return mHwManager.getDisplayModeName(2);
        }
        if (currentMode == 1) {
            return mHwManager.getDisplayModeName(1);
        }
        return mHwManager.getDisplayModeName(0);
    }
}
