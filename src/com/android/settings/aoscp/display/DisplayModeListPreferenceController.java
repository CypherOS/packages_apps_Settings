...
public class DisplayModeListPreferenceController extends BasePreferenceController
    ...
    public int getAvailabilityStatus() {
        return mHwManager.isSupported(DeviceHardwareManager.FEATURE_DISPLAY_MODES) ? 
	...
	private boolean updateDisplayModes() {
        ...
        for (int i = 0; i < modes.length; i++) {
            ...
            entries[i] = getLocalizedString(
                    mContext.getResources(), modes[i].name, DISPLAY_MODE_TITLE);

            // Populate summary
            String summary = getLocalizedString(
                    mContext.getResources(), modes[i].name, DISPLAY_MODE_SUMMARY);
            if (summary != null) {
	...
	public String getStringForResourceName(final Resources res, final String resourceName,
        ...
        if (resId <= 0) {
            Log.e("DisplayModeListPreferenceController", "No resource found for " + resourceName);
            return defaultValue;
        } else {
