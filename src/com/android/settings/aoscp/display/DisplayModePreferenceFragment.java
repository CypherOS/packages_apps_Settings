...
public class DisplayModePreferenceFragment extends RadioButtonPickerFragment {
            ...
            for (int colorMode : availableModes) {
                if (colorMode == 0) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(R.string.display_mode_option_normal),
                                KEY_DISPLAY_MODE_DEFAULT, true /* enabled */));
                } else if (colorMode == 1) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(R.string.display_mode_option_vivid),
	...
	public int getModeName(int mode) {
		int modeName = 0;
		try{
			modeName = Integer.valueOf(mHwManager.getDisplayModeName(mode));
