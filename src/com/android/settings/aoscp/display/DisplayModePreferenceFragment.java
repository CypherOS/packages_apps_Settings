...
public class DisplayModePreferenceFragment extends RadioButtonPickerFragment {
    ...
    protected List<? extends CandidateInfo> getCandidates() {
        ...
        final DisplayMode[] availableModes = mController.getAvailableModes();
            ...
            for (int colorMode : availableModes) {
                if (colorMode == 0) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(Integer.valueOf(mController.getModeEntry(0))),
                                KEY_DISPLAY_MODE_DEFAULT, true /* enabled */));
                } else if (colorMode == 1) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(Integer.valueOf(mController.getModeEntry(1))),
                                KEY_DISPLAY_MODE_1, true /* enabled */));
                } else if (colorMode == 2) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(Integer.valueOf(mController.getModeEntry(2))),
                                KEY_DISPLAY_MODE_2, true /* enabled */));
                } else if (colorMode == 3) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(Integer.valueOf(mController.getModeEntry(3))),
    ...
    protected String getDefaultKey() {
        final DisplayMode colorMode = mController.getCurrentMode();
        if (colorMode == 3) {
    ...
    protected boolean setDefaultKey(String key) {
        switch (key) {
            case KEY_DISPLAY_MODE_DEFAULT:
                mController.setMode(0);
                break;
            case KEY_DISPLAY_MODE_1:
                mController.setMode(1);
                break;
            case KEY_DISPLAY_MODE_2:
                mController.setMode(2);
                break;
