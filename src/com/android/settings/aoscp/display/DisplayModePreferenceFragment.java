            ...
            for (int colorMode : availableModes) {
                if (colorMode == 0) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(mController.getModeEntry(0).toString()),
                                KEY_DISPLAY_MODE_DEFAULT, true /* enabled */));
                } else if (colorMode == 1) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(mController.getModeEntry(1).toString()),
                                KEY_DISPLAY_MODE_1, true /* enabled */));
                } else if (colorMode == 2) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(mController.getModeEntry(2).toString()),
                                KEY_DISPLAY_MODE_2, true /* enabled */));
                } else if (colorMode == 3) {
                    candidates.add(new DisplayModeCandidateInfo(
                                ctx.getText(mController.getModeEntry(3).toString()),
    ...
    protected boolean setDefaultKey(String key) {
        switch (key) {
            case KEY_DISPLAY_MODE_DEFAULT:
                mController.setMode(DisplayMode[0]);
                break;
            case KEY_DISPLAY_MODE_1:
                mController.setMode(DisplayMode[1]);
                break;
            case KEY_DISPLAY_MODE_2:
                mController.setMode(DisplayMode[2]);
                break;
            case KEY_DISPLAY_MODE_3:
                mController.setMode(DisplayMode[3]);
                break;
        }
        return true;
    }
