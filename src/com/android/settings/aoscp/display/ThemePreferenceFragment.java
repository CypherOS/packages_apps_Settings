...
public class ThemePreferenceFragment extends DashboardFragment {
	...
	private PackageManager mPackageManager = new mContext.getPackageManager();;
	...
	private boolean isChangeableOverlay(String packageName) {
        try {
            PackageInfo pkgMgr = mPackageManager.getPackageInfo(packageName, 0);
            return pkgMgr != null && !pkgMgr.isStaticOverlay;
    ...
    public String getTheme() {
        try {
            List<OverlayInfo> infos = mThemeOM.getOverlayInfosForTarget("android",
                    UserHandle.myUserId());
