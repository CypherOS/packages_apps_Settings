...
public class SecurityPatchPreferenceController extends AbstractPreferenceController implements
    ...
    /* Returns the security patch level via string overlay override */
    private static String mPatchOverride;
    /* Returns the security patch level via prop override */
    private static String mPatchOverrideProp;
    ...
    public SecurityPatchPreferenceController(Context context) {
        ...
        mPatchOverride = mContext.getResources().getString(R.string.config_overridenVendorSecurityPatch);
        mPatchOverrideProp = SystemProperties.get("ro.vendor.override.security_patch", "");
    }
    ...
    public static String getSecurityPatch() {
        if (!TextUtils.isEmpty(mPatchOverrideProp)) {
			if (!"".equals(mPatchOverrideProp)) {
                try {
                    SimpleDateFormat template = new SimpleDateFormat("yyyy-MM-dd");
                    Date patchDate = template.parse(mPatchOverrideProp);
                    String format = DateFormat.getBestDateTimePattern(Locale.getDefault(), "dMMMMyyyy");
                    mPatchOverrideProp = DateFormat.format(format, patchDate).toString();
                } catch (ParseException e) {}
                return mPatchOverrideProp;
            } else {
                return null;
            }
		} else {
			if (!TextUtils.isEmpty(mPatchOverride)) {
